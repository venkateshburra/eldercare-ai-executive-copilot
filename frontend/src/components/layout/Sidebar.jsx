...
export const Sidebar = ({ isOpen, onClose }) => {
  ...
  const navItems = [
    {
      ...
      items: [
        ...
        { name: "Knowledge Search", path: "/knowledge-search", icon: FiSearch },
        { name: "Scenario Builder", path: "/scenario-builder", icon: FiSliders },
        { name: "Briefings & Decisions", path: "/decisions", icon: FiCheckSquare },
        { name: "Source-Cited AI Q&A", path: "/ai-qa", icon: FiMessageSquare },
      ],
    },
    {
      label: "Analytics & Oversight",
      items: [
        { name: "Sensitivity Analysis", path: "/sensitivity-analysis", icon: FiSliders },
        { name: "Outcome Review", path: "/outcome-review", icon: FiTarget },
        { name: "Reports & Analytics", path: "/reports", icon: FiBarChart2 },
        { name: "Notification Center", path: "/notifications", icon: FiBell },
      ],
    },
    {
      ...
      items: [
        ...
        { name: "Audit & Settings", path: "/settings", icon: FiSettings },
      ],
    },
  ];
  ...
  return (
    <>
      {/* Mobile Backdrop Overlay (only rendered when drawer is open on small screens) */}
      {isOpen && (
        <div
          onClick={onClose}
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-screen overflow-y-auto transition-transform duration-200 ease-in-out lg:z-30 lg:translate-x-0 ${
              ...
              <div className="space-y-3">
                {group.items.map((item) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-3 py-2 rounded-md text-sm md:text-[14px] font-medium transition-colors ${
                        isActive
